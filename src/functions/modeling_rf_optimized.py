...
def prepare_data(df):
    ...
    # Limit FunctLocDescrip. to top 10 categories
    top_categories = df['FunctLocDescrip.'].value_counts().nlargest(10).index
    df['FunctLocDescrip.'] = df['FunctLocDescrip.'].where(df['FunctLocDescrip.'].isin(top_categories), 'Other')
...
def train_model(X, y):
    ...
    # Train Random Forest with custom class weight
    ...
    # Generate plots
    os.makedirs('../results/plots/modeling/', exist_ok=True)
    ...
    plt.savefig('../results/plots/modeling/confusion_matrix_rf.png')
    plt.close()
    ...
    plt.savefig('../results/plots/modeling/roc_curve_rf.png')
    plt.close()
    ...
    plt.ylabel('Feature')
    plt.savefig('../results/plots/modeling/feature_importance_rf.png')
    plt.close()

    return model

def save_model(model, encoder, file_path='../models/rf_model_optimized.pkl'):
    import joblib
